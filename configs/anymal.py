...
ANYDRIVE_3_MLP_ACTUATOR_CFG = ActuatorNetMLPCfg(
    joint_names_expr=[".*HAA", ".*HFE", ".*KFE"],
    network_file=f"/home/william/Desktop/FSR_Project/ActNet_100Hz.pt",
    saturation_effort=120.0,
    effort_limit=80.0,
    velocity_limit=7.5,
    # we don't use stiffness and damping for actuator net
    pos_scale = 0.26779344707,    # scale for position error inputs
    vel_scale = 4.77956061758,    # scale for joint velocity inputs
    torque_scale = 1.0, # scale for torque outputs

    input_order = "pos_vel",    # or "vel_pos" depending on how your model expects inputs
    input_idx = [0, 1, 2],      # current + 2 historical steps (e.g., t, t-2, t-4) (because simulation is 200Hz while NN is 100Hz)
)
"""Configuration for ANYdrive 3.0 (used on ANYmal-C) with MLP actuator model."""
...
ANYMAL_B_CFG = ArticulationCfg(
    ...
    actuators={"legs": ANYDRIVE_3_MLP_ACTUATOR_CFG},
...
ANYMAL_C_CFG = ArticulationCfg(
    ...
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(0.0, 0.0, 0.6),
        rot=(0.7073883, 0, 0, 0.7068252), # With this orientation, the robot spawns looking at world's y-axis
        joint_pos={
            ".*HAA": 0.0,  # all HAA
            ".*F_HFE": 0.4,  # both front HFE
            ".*H_HFE": -0.4,  # both hind HFE
